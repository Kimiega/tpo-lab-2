...
public class Ln extends Function {
    ...
    public BigDecimal count(BigDecimal x) {
        ...
        if (x.subtract(BigDecimal.ONE).abs().compareTo(BigDecimal.ONE) <= 0) {
            do {
                prev = current;
                BigDecimal term = BigDecimal.valueOf(Math.pow(-1, iter - 1)).multiply(BigDecimal.valueOf(Math.pow(x.subtract(BigDecimal.ONE).doubleValue(), iter))).divide(BigDecimal.valueOf(iter), MathContext.DECIMAL128);
